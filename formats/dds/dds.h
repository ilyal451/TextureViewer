...
typedef struct _DDS_HEADER_DX10
{
...
typedef struct PixelFormatInfo
{
	char* pszPixelFormat;
	int nStreams;
	StreamInfo_t stream[MAX_STREAMS];
	int iPaletteSize;
	int dxgiFormat;
	DDS_PIXELFORMAT pf;
	int eInputGamma;
	int flags;
	char aiNumChannelBits[NUM_CHANNELS];
} PixelFormatInfo_t;
...
pixel_format_t* GetDXGIFormat( int dxgiFormat );
void DXGI_DecodePixels( int eDXGIFormat, RGBQuad_t* pixels, int iWidth, int iHeight, void* data );
